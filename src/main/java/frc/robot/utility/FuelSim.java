...
public class FuelSim {
  ...
  protected static final double FRICTION =
      0.2; // proportion of horizontal vel to lose per sec while on ground
   ...
   *
   * @param logKey AdvantageKit log key for fuel positions (logged as an array of {@link
   *     Translation3d}).
   */
  ...
  /**
   * Gets the positions of all fuel on the field
   *
   * @return array of Translation3d positions
   */
  ...
  /**
   * Gets the number of fuel currently on the field
   *
   * @return fuel count
   */
  ...
  /**
   * Spawns a fuel onto the field from a shooter at a specific position on the robot, accounting for
   * robot movement
   *
   * @param launchVelocity Initial launch velocity
   * @param hoodAngle Hood angle where 0 is launching horizontally and 90 degrees is launching
   *     straight up
   * @param shooterTransform Robot-relative transform of the shooter (position and yaw)
   * @throws IllegalStateException if robot is not registered
   */
  public void launchFuel(
      LinearVelocity launchVelocity, Angle hoodAngle, Transform3d shooterTransform) {
    if (robotPoseSupplier == null || robotFieldSpeedsSupplier == null) {
      throw new IllegalStateException("Robot must be registered before launching fuel.");
    }

    Pose3d launchPose = new Pose3d(this.robotPoseSupplier.get()).plus(shooterTransform);
    ChassisSpeeds fieldSpeeds = this.robotFieldSpeedsSupplier.get();

    double fieldYaw = launchPose.getRotation().getZ();
    double horizontalVel = Math.cos(hoodAngle.in(Radians)) * launchVelocity.in(MetersPerSecond);
    double verticalVel = Math.sin(hoodAngle.in(Radians)) * launchVelocity.in(MetersPerSecond);
